...
const UserInvoice = ({ userId }) => {
  ...
  const invoiceHandler = async () => {
    try {
      const response = await fetch(`https://next-resort-project-ten.vercel.app/api/users/${userId}`);
  ...
  const calculateDays = (startDate, endDate) => {
    ...
    const end = new Date(endDate);
     // reset time to avoid time issues
  start.setHours(0, 0, 0, 0);
  end.setHours(0, 0, 0, 0);
    const diffTime = end - start;
  const diffDays = diffTime / (1000 * 60 * 60 * 24);

  // +1 because booking is inclusive
  return diffDays + 1;
  };
  ...
  const deleteBooking = async (bookingId) => {
    ...
    try {
      const response = await fetch(`https://next-resort-project-ten.vercel.app/api/users/${bookingId}`, {
        method: 'DELETE',
      });
  ...
  return (
    ...
    {invoice.bookings.length > 0 ? (
      invoice.bookings.map((item) => {
        ...
        const totalAmount = days * Number(item.price);
  
        return (
          <div key={item._id} className="booking-item">
            <h3>{item.productName}</h3>
            <p>Booking dates from {formatDate(item.startDate)} to {formatDate(item.endDate)}</p>
